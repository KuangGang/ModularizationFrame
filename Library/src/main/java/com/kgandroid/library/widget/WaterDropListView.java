...
public class WaterDropListView extends ListView implements OnScrollListener,
	...
	public boolean isRefreshing(){
		return mHeaderView.getCurrentState() == WaterDropListViewHeader.STATE.refreshing;
	}
...
//		if (height == 0) {
//			// not visible.
//			Log.e("tik", "resetHeaderHeight --> 111");
//			return;
//		}
		// refreshing and header isn't shown fully. do nothing.
		if (mHeaderView.getCurrentState() == WaterDropListViewHeader.STATE.refreshing
				&& height <= mHeaderView.getStretchHeight()) {
//			Log.e("tik", "resetHeaderHeight --> 222");
			return;
		}
...
//			Log.e("tik", "resetHeaderHeight --> 333");
		}
//		Log.e("tik","finalHeight="+finalHeight+",height="+height);
		mScrollBack = ScrollBack.header;
		mScroller.startScroll(0, height, 0, finalHeight - height,
				SCROLL_DURATION);
		// trigger computeScrol
		invalidate();
	}

	private void resetHeaderHeightImmediately(){
		int height = mHeaderView.getVisiableHeight();
//		if (height == 0) {
//			// not visible.
//			Log.e("tik", "resetHeaderHeight --> 111");
...
//		Log.e("tik","finalHeight="+finalHeight+",height="+height);
	...
	private void startLoadMore() {
		...
		if (mListViewListener != null && !isLock) {
			lock();
		...
		case MotionEvent.ACTION_MOVE:
			if (!isLock) {
				...
				if (getFirstVisiblePosition() == 0
					...
					Log.e("tik","ACTION_MOVE--------1");
					// the first item is showing, header has shown or pull down.
					updateHeaderHeight(deltaY / OFFSET_RADIO);
					invokeOnScrolling();
				} else if (getLastVisiblePosition() == mTotalItemCount - 1
						&& ((mFooterView.getBottomMargin() > 0 && deltaY != 0) || deltaY < 0)) {
					Log.e("tik","ACTION_MOVE--------2");
			...
			default:
				...
				boolean isHeaderAnim = false;
				if (getFirstVisiblePosition() == 0) {
					if(mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA){
						resetHeaderHeightImmediately();
					}else{
						resetHeaderHeight();
						isHeaderAnim = true;
					}
				...
				if (getLastVisiblePosition() == mTotalItemCount - 1) {
					...
					}, isHeaderAnim ? SCROLL_DURATION : 50);
				}
				break;
		}
