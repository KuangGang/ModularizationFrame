...
public final class CrashUtils
    ...
    public boolean init() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            File baseCache = AppContextUtils.getContext().getExternalCacheDir();
            if (baseCache == null) return false;
            crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
        } else {
            File baseCache = AppContextUtils.getContext().getCacheDir();
            if (baseCache == null) return false;
            crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
        }
        try {
            PackageInfo pi = AppContextUtils.getContext().getPackageManager().getPackageInfo(AppContextUtils.getContext().getPackageName(), 0);
